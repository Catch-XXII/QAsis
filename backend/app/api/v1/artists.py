...
from app.db.schemas.artist_schema import ArtistSchema
...
@router.get("/artists")
async def get_artists(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
...
async def get_artists_by_name(
    ...
    total_stmt = select(func.count()).select_from(Artist).where(Artist.name.ilike(f"%{name}%"))
    total_result = await db.execute(total_stmt)
    total = total_result.scalar()

    data_stmt = select(Artist).where(Artist.name.ilike(f"%{name}%")).offset(skip).limit(limit)
    data_result = await db.execute(data_stmt)
    rows = data_result.scalars().all()
